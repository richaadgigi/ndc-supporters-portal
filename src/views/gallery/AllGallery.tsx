...
import { Renew, Download, Time, UserAvatar } from '@carbon/icons-react';
...
const AllGallery = () => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try { handleResponse(await galleryService.publicGetAll({ page: currentPage, size: pageSize, ...(tagRef.current && { tags: tagRef.current }), ...(supportGroupId && { support_group_unique_id: supportGroupId }) })); }
  ...
  const searchItems = useCallback(async (query: string) => {
    ...
    setLoading(true); setFetchError('');
    try { handleResponse(await galleryService.publicGetAll({ search: query, page: currentPage, size: pageSize, ...(tagRef.current && { tags: tagRef.current }), ...(supportGroupId && { support_group_unique_id: supportGroupId }) })); }
  ...
  const filterItems = useCallback(async (range: { start_date: string; end_date: string }) => {
    setLoading(true); setFetchError('');
    try { handleResponse(await galleryService.publicGetAll({ start_date: range.start_date, end_date: range.end_date, page: currentPage, size: pageSize, ...(tagRef.current && { tags: tagRef.current }), ...(supportGroupId && { support_group_unique_id: supportGroupId }) })); }
  ...
  return (
    <div>
      <Navbar title="Gallery" subtitle="Browse support group gallery" />
      <div className="xui-py-1-half">
        ...
        {loading ? (
          ...
          <div className="xui-bg-white xui-bdr-rad-half xui-overflow-hidden" style={{ border: '1px solid var(--neutral-200)' }}>
            <EmptyState title="No images found" message={searchQuery ? "No images match your search." : hasActiveFilters ? "No images match your filters." : "There are no gallery images to display."} />
          </div>
        ) : (
          <>
            <div className="xui-d-grid xui-grid-col-1 xui-md-grid-col-2 xui-lg-grid-col-3 xui-grid-gap-1">
              {items.map((item) => (
                <div
                  ...
                  onClick={() => router.push(`/dashboard/gallery/view/${item.unique_id}`)}
